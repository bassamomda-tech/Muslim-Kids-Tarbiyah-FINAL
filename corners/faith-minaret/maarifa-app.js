/* ════ كيف عَرَفْنا الله؟ — رِحلةُ المَعرِفة · engine ════ */
(function(){
  const S = window.MAARIFA_STATIONS || [];
  const K = { done:'maarifaDone', journal:'maarifaJournal', name:'maarifaName', acts:'maarifaActs', lang:'bunyanLang' };
  ...
  function renderMap(){
      ...
      el.innerHTML=`
        ...
        <div class="s-label">
          ...
          <span class="go">${locked?t('قريباً ✨','Coming soon ✨')
            :(d?t('مَعروفة ✓ — أعِد الزِّيارة','Known ✓ — revisit'):t('اصعَد الدَّرَجة ←','Climb the step →'))}</span>
    ...
    fin.innerHTML=`<div class="cup" id="cupBtn">🏆</div><div class="ft">${t('وِسامُ العارِفِ بِالله','Knower of Allah Medal')}</div>`;
    map.appendChild(fin);
  ...
  function updateMeter(){
    ...
    let lbl;
    if(da===0) lbl=t('ابدأ الصُّعود… دَرَجةً دَرَجةً نَعرِفُ الله','Start climbing… step by step we come to know Allah');
    else if(da<ac) lbl=t(`أحسَنت! صَعِدتَ ${da} مِن ${ac} دَرَجات`,`Great! You\'ve climbed ${da} of ${ac} steps`);
    else lbl=t('🎉 بَلَغتَ القِمّة! عَرَفتَ اللهَ فَأحبَبتَه','🎉 You reached the summit! You knew Allah and loved Him');
  ...
  function renderCollections(){
    ...
    $('#collStickers').innerHTML=`<div class="ci">🎟️</div><div class="ct">${t('أوسِمةُ المَعرِفة','Knowledge Badges')}</div><div class="cn">${da} ${t('وِسام','badges')}</div>`;
    $('#collNames').innerHTML=`<div class="ci">🕋</div><div class="ct">${t('جِدارُ الأسماء','Names Wall')}</div><div class="cn">${da} ${t('اسم','names')}</div>`;
    $('#collKeys').innerHTML=`<div class="ci">🔑</div><div class="ct">${t('دَرَجاتُ المَعرِفة','Steps of Knowing')}</div><div class="cn">${da} / ${totalActive}</div>`;
    $('#collCert').innerHTML=`<div class="ci">📜</div><div class="ct">${t('شَهادةُ العارِف','Knower Diploma')}</div><div class="cn">${t('اطبَع شَهادَتَك ←','Print yours →')}</div>`;
  }
    ...
    $('#rBody').innerHTML=`
      ...
      <div class="blocks">
        ...
        <div class="block myth">
          ...
          <div class="flip" id="flipCard">
            <div class="flip-inner">
              <div class="flip-face flip-front"><div class="ft-tag">${t('الشُّبهة','The Doubt')}</div><div>${st.myth.claim[L]}</div></div>
        ...
        <div class="block journal">
          ...
          <div class="journal">
            <textarea id="journalBox" placeholder="${t('اكتُب ما عَرَفتَهُ عَن رَبِّكَ اليوم…','Write what you learned about your Lord today…')}">${(journal[st.id]||'').replace(/</g,'&lt;')}</textarea>
      ...
      <div class="r-foot">
        <button class="stampbtn ${d?'done':''}" id="stampBtn">
          ${d?t('✓ دَرَجةٌ مَعروفة','✓ Step known'):t('🎖️ اصعَد هذه الدَّرَجة','🎖️ Climb this step')}
        </button>
        ${nextExists(i)?`<div class="r-next-hint">${t('جاهِزٌ لِلتّالية؟','Ready for the next?')} <button id="nextBtn">${t('الدَّرَجةُ التّالية ←','Next step →')}</button></div>`:''}
      </div>`;
    ...
    [...Lc.children,...Rc.children].forEach(el=>{
      el.onclick=()=>{
        ...
        if(sel.dataset.pair===el.dataset.pair){
          ...
          if(matched===pairs.length){ doneEl.textContent=t('🎉 أحسَنت! كُلُّ شَيءٍ في مَوضِعِه — هكذا نَعرِفُ رَبَّنا بِالعَقلِ والدَّليل.','🎉 Brilliant! Everything in its place — this is how we know our Lord with reason and proof.'); }
        } else {
  ...
  function openDrawer(kind){
    ...
    if(kind==='stickers'){
      title.textContent=t('🎟️ أوسِمةُ المَعرِفة','🎟️ Knowledge Badges');
      ...
      title.textContent=t('🔑 دَرَجاتُ المَعرِفة','🔑 Steps of Knowing');
      body.innerHTML=S.filter(s=>!s.locked).map(s=>{
        const d=done.has(s.id);
        return `<div class="list-card ${d?'':'locked'}"><div class="lci">${d?'🗝️':'🔒'}</div><div>
          <div class="lct">${d?'«'+s.proof[lang]+'»':t('دَرَجةٌ مُقفَلة','Locked step')}</div>
          <div class="lcs">${d?t('مِن مَحطّة: ','From: ')+s.title[lang]:t('أكمِل المَحطّةَ لِتَنالَ الدَّرَجة','Complete the station to earn the step')}</div></div></div>`;
      }).join('');
    }
  ...
  function openCertificate(){
    ...
    if(doneStations.length===0){
      statement=t('ابدأ رِحلَتَك بِصُعودِ أوّلِ دَرَجة، وستَظهَرُ هُنا دَرَجاتُ مَعرِفةِ اللهِ التي صَعِدتَها بِعَقلِك.',
                  'Begin by climbing your first step, and the steps of knowing Allah you climb with your mind will appear here.');
    } else {
      statement=t(
        `صَعِدَ هذا العارِفُ الصَّغيرُ <b>${doneStations.length}</b> مِن ${total} دَرَجاتٍ في رِحلةِ المَعرِفة، فَعَرَفَ بِعَقلِهِ — على نَهجِ أهلِ السُّنّةِ والجَماعة — أنَّ لِلكَونِ خالِقاً واحِداً حَكيماً، وأنَّ اللهَ عَرَّفَنا بِنَفسِهِ عَبرَ رُسُلِهِ وكِتابِهِ وأسمائِهِ الحُسنى، فَعَرَفَهُ فَأحبَّهُ وعَبَدَهُ وَحدَه.`,
        `This young knower climbed <b>${doneStations.length}</b> of ${total} steps in the Journey of Knowing, learning with their own mind — upon the way of Ahl al-Sunnah — that the universe has one Wise Creator, that Allah introduced Himself through His messengers, His book, and His beautiful names, so they knew Him, loved Him, and worshipped Him alone.`);
    }
    const keysHTML = doneStations.map(s=>`<span class="ck">🗝️ ${s.proof[lang]}</span>`).join('');
    $('#certBox').innerHTML=`
      <div class="cert-seal">🕋</div>
      <div class="cert-kick">${t('رِحلةُ المَعرِفة · منارةُ العقيدة','The Journey of Knowing · Faith Minaret')}</div>
      <div class="cert-title">${t('شَهادةُ العارِفِ بِالله','Certified Knower of Allah')}</div>
  ...
  /* ───────── FX ───────── */
  function celebrate(){
    const c=$('#confetti'), cols=['#F0C24B','#E1A100','#0984E3','#8E44AD','#00B894','#E84393'];
    for(let k=0;k<60;k++){
