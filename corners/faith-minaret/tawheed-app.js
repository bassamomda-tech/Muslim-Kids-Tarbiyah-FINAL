/* ════ رحلة التوحيد — City of Tawheed · engine ════ */
(function(){
  const S = window.TAWHEED_STATIONS || [];
  const K = { done:'tawheedDone', journal:'tawheedJournal', name:'tawheedName', acts:'tawheedActs', lang:'bunyanLang' };
  ...
  function renderMap(){
      ...
      el.innerHTML=`
        ...
        <div class="s-label">
          ...
          <span class="go">${locked?t('قريباً ✨','Coming soon ✨')
            :(d?t('مَختومة ✓ — أعِد الزِّيارة','Stamped ✓ — revisit'):t('ادخُل ←','Enter →'))}</span>
    ...
    fin.innerHTML=`<div class="cup" id="cupBtn">🏆</div><div class="ft">${t('وِسامُ مُوَحِّدِ القَصر','Palace Tawheed Medal')}</div>`;
    map.appendChild(fin);
  ...
  function updateMeter(){
    ...
    let lbl;
    if(da===0) lbl=t('ادخُل القَصرَ… التَّوحيدُ يَنتَظِر','Enter the palace… Tawheed awaits');
    else if(da<ac) lbl=t(`نورُ التَّوحيدِ يَزدادُ! ${da} مِن ${ac} مَحطّات`,`The light of Tawheed is growing! ${da} of ${ac} stations`);
    else lbl=t('🎉 أتمَمتَ قَصرَ المَلِكِ الواحِد!','🎉 You completed the Palace of the One King!');
  ...
  function renderCollections(){
    ...
    $('#collStickers').innerHTML=`<div class="ci">🎟️</div><div class="ct">${t('ألبومُ الأوسِمة','Sticker Album')}</div><div class="cn">${da} ${t('وِسام','badges')}</div>`;
    $('#collNames').innerHTML=`<div class="ci">🕋</div><div class="ct">${t('جِدارُ الأسماء','Names Wall')}</div><div class="cn">${da} ${t('اسم','names')}</div>`;
    $('#collKeys').innerHTML=`<div class="ci">🔑</div><div class="ct">${t('بَراهينُ التَّوحيد','Proofs of Tawheed')}</div><div class="cn">${da} / ${totalActive}</div>`;
    $('#collCert').innerHTML=`<div class="ci">📜</div><div class="ct">${t('شَهادةُ المُوَحِّد','Tawheed Certificate')}</div><div class="cn">${t('اطبَع شَهادَتَك ←','Print yours →')}</div>`;
  }
    ...
    $('#rBody').innerHTML=`
      ...
      <div class="blocks">
        ...
        <div class="block journal">
          ...
          <div class="journal">
            <textarea id="journalBox" placeholder="${t('اكتُب ما تَعَلَّمتَهُ عَن تَوحيدِ اللهِ اليوم…','Write what you learned about the oneness of Allah today…')}">${(journal[st.id]||'').replace(/</g,'&lt;')}</textarea>
  ...
  function openDrawer(kind){
    ...
    if(kind==='stickers'){
      ...
      title.textContent=t('🔑 بَراهينُ التَّوحيد','🔑 Proofs of Tawheed');
      body.innerHTML=S.filter(s=>!s.locked).map(s=>{
        const d=done.has(s.id);
        return `<div class="list-card ${d?'':'locked'}"><div class="lci">${d?'🗝️':'🔒'}</div><div>
          <div class="lct">${d?'«'+s.proof[lang]+'»':t('بُرهانٌ مُقفَل','Locked proof')}</div>
          <div class="lcs">${d?t('مِن مَحطّة: ','From: ')+s.title[lang]:t('أكمِل المَحطّةَ لِتَنالَ البُرهان','Complete the station to earn the proof')}</div></div></div>`;
      }).join('');
    }
  ...
  function openCertificate(){
    ...
    if(doneStations.length===0){
      statement=t('ابدأ رِحلَتَك بِخَتمِ أوّلِ مَحطّة، وستَظهَرُ هُنا بَراهينُ التَّوحيدِ التي تَعَلَّمتَها.',
                  'Begin by stamping your first station, and the proofs of Tawheed you learn will appear here.');
    } else {
      statement=t(
        `أتَمَّ هذا المُوَحِّدُ الصَّغيرُ <b>${doneStations.length}</b> مِن ${total} مَحطّاتٍ في قَصرِ المَلِكِ الواحِد، فتَعَلَّمَ بِعَقلِهِ — على نَهجِ أهلِ السُّنّةِ والجَماعة — أنَّ اللهَ واحِدٌ أحَدٌ لا شَريكَ لَه، اقتِداءً بِالنَّبِيِّ إبراهيمَ والإمامِ الباقِلّاني.`,
        `This young believer completed <b>${doneStations.length}</b> of ${total} stations in the Palace of the One King, learning with their own mind — upon the way of Ahl al-Sunnah — that Allah is One, Unique, with no partner, following Prophet Ibrahim and Imam Al-Baqillani.`);
    }
    const keysHTML = doneStations.map(s=>`<span class="ck">🗝️ ${s.proof[lang]}</span>`).join('');
    $('#certBox').innerHTML=`
      <div class="cert-seal">🕋</div>
      <div class="cert-kick">${t('رِحلةُ التَّوحيد · منارةُ العقيدة','Journey of Tawheed · Faith Minaret')}</div>
      <div class="cert-title">${t('شَهادةُ مُوَحِّدِ القَصر','Certified Palace Believer')}</div>
  ...
  /* ───────── FX ───────── */
  function celebrate(){
    const c=$('#confetti'), cols=['#E8B530','#6C5CE7','#27AE60','#2980B9','#F5CC5A','#9B59B6'];
    for(let k=0;k<60;k++){
