...
(function(){
  ...
  function speak(button, text){
    ...
    if(activeBtn===button){ stop(); return; }
    stop();                       // stop any other block that's currently playing
    if(!text) return;
    activeBtn=button; button.innerHTML=label(true);
    // Prefer natural cloud voice when configured; fall back to browser voices.
    if(window.MKTTS && MKTTS.enabled){
      MKTTS.speak(text, L()).then(function(){ reset(); }).catch(function(){ browserSpeak(text); });
      return;
    }
    browserSpeak(text);
  }
  ...
  var TARGETS=[
    { hdr:'#rBody .block.wow h3, #rBody .block.treasure h3, #rBody .blocks .block h3',
      txt:['.block.wow p','.block.treasure .maryam p','.block.treasure p','.blocks .block p'] },
