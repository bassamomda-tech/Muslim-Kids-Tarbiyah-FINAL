/* ════ سِلسِلةُ الأمان بالله — Chain of Security with Allah · engine ════ */
(function(){
  const S = window.AMAN_STATIONS || [];
  const K = { done:'amanDone', journal:'amanJournal', name:'amanName', acts:'amanActs', lang:'bunyanLang' };
  ...
  function renderMap(){
      ...
      el.innerHTML=`
        ...
        <div class="s-label">
          ...
          <span class="go">${locked?t('قريباً ✨','Coming soon ✨')
            :(d?t('آمِنة ✓ — أعِد الزِّيارة','Secured ✓ — revisit'):t('ادخُل المَحطّة ←','Enter →'))}</span>
    ...
    fin.innerHTML=`<div class="cup" id="cupBtn">🏆</div><div class="ft">${t('وِسامُ الأمانِ بِالله','Security with Allah Medal')}</div>`;
    map.appendChild(fin);
  ...
  function updateMeter(){
    ...
    let lbl;
    if(da===0) lbl=t('ابدأ السِّلسِلة… كُلُّ حَلقةٍ تَزيدُ قَلبَكَ أماناً','Begin the chain… each link adds peace to your heart');
    else if(da<ac) lbl=t(`أحسَنت! بَنَيتَ ${da} مِن ${ac} حَلَقاتِ الأمان`,`Great! You\'ve built ${da} of ${ac} links of security`);
    else lbl=t('🎉 اكتَمَلَ حِصنُ السَّكينة! قَلبُكَ مُمتَلِئٌ بِالله','🎉 The fortress of peace is complete! Your heart is full of Allah');
  ...
  function renderCollections(){
    ...
    $('#collStickers').innerHTML=`<div class="ci">🎟️</div><div class="ct">${t('أوسِمةُ الأمان','Security Badges')}</div><div class="cn">${da} ${t('وِسام','badges')}</div>`;
    $('#collNames').innerHTML=`<div class="ci">🕋</div><div class="ct">${t('جِدارُ الأسماء','Names Wall')}</div><div class="cn">${da} ${t('اسم','names')}</div>`;
    $('#collKeys').innerHTML=`<div class="ci">🔑</div><div class="ct">${t('مَفاتيحُ الطُّمَأنينة','Keys of Tranquility')}</div><div class="cn">${da} / ${totalActive}</div>`;
    $('#collCert').innerHTML=`<div class="ci">📜</div><div class="ct">${t('شَهادةُ الأمان','Security Diploma')}</div><div class="cn">${t('اطبَع شَهادَتَك ←','Print yours →')}</div>`;
  }
  ...
  function openStation(i){
    ...
    const H={
      ...
      key:  t('مِفتاحُ الطُّمَأنينة','Key of Tranquility'),
      dua:  t('دُعاءٌ تَحفَظُه','A Dua to Memorize'),
      mission:t('مُهِمّةُ المَحطّة',"Station Mission"),
      journal:t('دَفترُ الأمان','Security Journal'),
    ...
    $('#rBody').innerHTML=`
      ...
      <div class="blocks">
        ...
        <div class="block myth">
          ...
          <div class="flip" id="flipCard">
            <div class="flip-inner">
              <div class="flip-face flip-front"><div class="ft-tag">${t('وَسوَسةُ القَلَق','The Worry')}</div><div>${st.myth.claim[L]}</div></div>
              <div class="flip-face flip-back"><div class="ft-tag">${t('الطُّمَأنينة','The Peace')}</div><div>${st.myth.bust[L]}</div></div>
            </div>
          </div>
        ...
        <div class="block journal">
          ...
          <div class="journal">
            <textarea id="journalBox" placeholder="${t('اكتُب ما طَمأنَ قَلبَكَ اليوم…','Write what gave your heart peace today…')}">${(journal[st.id]||'').replace(/</g,'&lt;')}</textarea>
      ...
      <div class="r-foot">
        <button class="stampbtn ${d?'done':''}" id="stampBtn">
          ${d?t('✓ حَلقةٌ آمِنة','✓ Link secured'):t('🛡️ أمِّن هذه الحَلقة','🛡️ Secure this link')}
        </button>
        ${nextExists(i)?`<div class="r-next-hint">${t('جاهِزٌ لِلتّالية؟','Ready for the next?')} <button id="nextBtn">${t('الحَلقةُ التّالية ←','Next link →')}</button></div>`:''}
      </div>`;
    ...
    [...Lc.children,...Rc.children].forEach(el=>{
      el.onclick=()=>{
        ...
        if(sel.dataset.pair===el.dataset.pair){
          ...
          if(matched===pairs.length){ doneEl.textContent=t('🎉 أحسَنت! كُلُّ شَيءٍ في مَوضِعِه — هكذا يَطمَئِنُّ القَلب.','🎉 Brilliant! Everything in its place — this is how the heart finds peace.'); }
        } else {
  ...
  function openDrawer(kind){
    ...
    if(kind==='stickers'){
      title.textContent=t('🎟️ أوسِمةُ الأمان','🎟️ Security Badges');
      ...
      title.textContent=t('🔑 مَفاتيحُ الطُّمَأنينة','🔑 Keys of Tranquility');
      body.innerHTML=S.filter(s=>!s.locked).map(s=>{
        const d=done.has(s.id);
        return `<div class="list-card ${d?'':'locked'}"><div class="lci">${d?'🗝️':'🔒'}</div><div>
          <div class="lct">${d?'«'+s.proof[lang]+'»':t('مِفتاحٌ مُقفَل','Locked key')}</div>
  ...
  function openCertificate(){
    ...
    if(doneStations.length===0){
      statement=t('ابدأ رِحلَتَك بِتَأمينِ أوّلِ حَلقة، وستَظهَرُ هُنا مَفاتيحُ الطُّمَأنينةِ التي جَمَعتَها.',
                  'Begin by securing your first link, and the keys of tranquility you gather will appear here.');
    } else {
      statement=t(
        `أتَمَّ هذا القَلبُ الصَّغيرُ تَأمينَ <b>${doneStations.length}</b> مِن ${total} حَلَقاتٍ في سِلسِلةِ الأمانِ بِالله، فَعَرَفَ — على نَهجِ أهلِ السُّنّةِ والجَماعة — أنَّ اللهَ مَعَهُ ويَحفَظُهُ ويَرزُقُهُ ويَرحَمُه، فَآمَنَ بِقَدَرِهِ ولِقائِهِ وأسمائِه، فَأنزَلَ اللهُ على قَلبِهِ سَكينةً تَحميهِ مِنَ الخَوفِ والقَلَقِ واليَأسِ ووَساوِسِ الشَّيطان.`,
        `This young heart secured <b>${doneStations.length}</b> of ${total} links in the Chain of Security with Allah, learning — upon the way of Ahl al-Sunnah — that Allah is with him, guards him, provides for him, and is merciful to him; believing in His decree, His meeting, and His names, so Allah sent down upon his heart a peace that protects him from fear, anxiety, despair, and Satan\'s whispers.`);
    }
    const keysHTML = doneStations.map(s=>`<span class="ck">🗝️ ${s.proof[lang]}</span>`).join('');
    $('#certBox').innerHTML=`
      <div class="cert-seal">🛡️</div>
      <div class="cert-kick">${t('سِلسِلةُ الأمانِ بِالله · منارةُ العقيدة','The Chain of Security with Allah · Faith Minaret')}</div>
      <div class="cert-title">${t('شَهادةُ القَلبِ المُطمَئِنّ','Certified Heart at Peace')}</div>
  ...
  /* ───────── FX ───────── */
  function celebrate(){
    const c=$('#confetti'), cols=['#1ABC9C','#16A085','#27AE60','#2980B9','#E8C66B','#1E8449'];
    for(let k=0;k<60;k++){
